...
def get_all_reports():
    ...
    if search:
        ...
        data_with_scores = [(r, score_model(model=r, term=term, attributes=[
                             "title", "source", "author", "published_at", "categories", "description", "text_summary"])) for r in data]
        ...
        def match_search(obj):
            if isinstance(obj, Wildfire):
                ...
                return score_model(model=obj, term=term, attributes=[
                    "title", "source", "author", "published_at", "categories", "description", "text_summary"])
